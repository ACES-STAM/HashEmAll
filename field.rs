...
impl Field {
    ...
    pub fn square(&self, left: &FieldElement) -> FieldElement {
        self.multiply(left, left)
    }
    ...
    pub fn mod_biguint(&self, x: &BigUint) -> BigUint {
        x % &self.modulus
    }
}
...
impl FieldElement {
    ...
    pub fn mod_biguint(&self, divisor: &BigUint) -> BigUint {
        &self.value % divisor
    }

    pub fn div_biguint(&self, divisor: &BigUint) -> FieldElement {
        let new_value = &self.value / divisor;
        FieldElement::new(new_value, self.field.clone())
    }

    pub fn to_biguint(&self) -> BigUint {
        self.value.clone()  // Adjust based on your internal representation
    }
}
