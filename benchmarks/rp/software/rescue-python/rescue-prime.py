...
class RescuePrime:
    ...
    def hash(self, input_element):
        ...
        # print(f'start state[{0}] - {state[0]}')
        # print(f'start state[{1}] - {state[1]}')
        # print(f'start alpha  -   {self.alpha}')
        ...
        for r in range(self.N):
            ...
            for i in range(self.m):
                for j in range(self.m):
                    mult_val = self.MDS[i][j] * state[j]
                    temp[i] = temp[i] + mult_val
           
            # constants
            state = [temp[i] + self.round_constants[2*r*self.m+i] for i in range(self.m)]
            # print("number of round const: ", len(self.round_constants))
...
element_value = FieldElement(4, this_field)
hash_value = rescue_prime.hash(element_value)

print('This is the hash value: ', hash_value)
